...
const char DEFAULT_WALLET_FILE_NAME[] = "doshwallet.wallet";
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  res = autorunSettings.contains("DoshWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("DoshWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.dosh.dosh scientistwallet");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("DoshWallet", appPath);
    } else {
      autorunSettings.remove("DoshWallet");
    }
#endif
  }
